...
> clangd would throw out an error: ```unknown argument: '-fstrict-volatile-bitfields'```; to avoid this you can do so by instructing clangd to remove that flag from the compile command. To do this create a file ```~/.config/clangd``` with the following contents:
>```
>CompileFlags:
