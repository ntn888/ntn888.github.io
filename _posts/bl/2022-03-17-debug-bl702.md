...
Note: As it is, GDB hangs on the message ```Starting program: <path_to_project>/build_out/main.elf``` and doesn't return to the ```(gdb)``` prompt. I'm trying to troubleshoot this issue and have open a ticket [here](https://bbs.bouffalolab.com/d/126-bl702-command-line-gdb-debug).
